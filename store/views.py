...
def submit_reviews(request,product_id):
    #this url will store the current page url
    url = request.META.get('HTTP_REFERER')
    if request.method == 'POST':

        try:
           
            #as we hvae user as a foriegn key in models ..we can use it as use__id
            reviews = ReviewRating.objects.get(user__id=request.user.id, product__id = product_id)
            #instance = reviews (will check if there already a review ..if there is..we will update it )
